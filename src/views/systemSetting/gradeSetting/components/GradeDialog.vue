<template>
  ...
  >
    <el-form :model="formData" ref="formRef" :rules="rules" label-position="top" v-loading="$store.state.isLoading">
      ...
      <el-form-item label="年级状态" prop="status">
...
<script>
import { addGradeService, updateGradeService } from '@/api/systemSetting.js';
const defaultData = {
  name: '',
  status: false,
};
  ...
  methods: {
    ...
    submit() {
      this.$refs.formRef.validate(async (validate) => {
        if (validate) {
          if (this.formData.id) {
            await updateGradeService(this.formData);
            this.$message.success('编辑年级成功');
          } else {
            await addGradeService(this.formData);
