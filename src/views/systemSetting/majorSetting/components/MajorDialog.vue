  ...
  >
    <el-form :model="formData" ref="formRef" :rules="rules" label-position="top" v-loading="$store.state.isLoading">
      <el-form-item label="专业名称" prop="name">
        <el-input v-model="formData.name" placeholder="请输入专业名称"></el-input>
      </el-form-item>
...
<script>
import { addMajorService, updateMajorService } from '@/api/systemSetting.js';
const defaultData = {
  name: '',
  status: false,
};
  ...
  methods: {
    ...
    submit() {
      this.$refs.formRef.validate(async (validate) => {
        if (validate) {
          if (this.formData.id) {
            await updateMajorService(this.formData);
            this.$message.success('编辑专业成功');
          } else {
            await addMajorService(this.formData);
